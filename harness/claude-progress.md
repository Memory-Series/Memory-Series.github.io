...
- 已知风险或未解决问题：
  ...
  - ~~Session 036 遗留的两条人工确认项~~ —— **2026-09-13 均已关闭**：①「默认偏好下产品影片确实自动播放」：用户明确**不再跟进**（reduced-motion 路径已自动验证「不自动播放」；默认路径属浏览器标准 `autoplay muted playsinline` 行为，且用户已在线上页面实际看到影片播放；headless chromium 无 H.264 解码，放弃人工复核）；② FAQ 中英视觉效果：已由线上真实浏览器渲染校验覆盖（`#faq` 四条目 + 展开后步骤完整 + 控制台 0 error），用户亦已过目
- **收尾确认（同日追加）**：用户问「当前上面的任务是不是已经收尾了」并指示影片自动播放确认「暂时不用做」→ 经核对，代码 / 部署 / harness 三条线均已收尾（工作区干净、HEAD 与 origin/main 同步 `6bebe4c`、两站已发布至含 a11y-001 + FAQ 的版本），**本批任务全部关闭，无进行中项**
...
- 提交记录：`641c887`（docs(harness): reconcile harness with actual project state，6 文件，+300 −137，纯文档），已推送 origin/main
